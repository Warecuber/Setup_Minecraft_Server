...
sudo dnf install htop glances neofetch -y
...
sudo mkdir $MC_SERVER_DIR/server
sudo chown -R root:mc_server_admin $MC_SERVER_DIR/server
sudo chmod 770 -R $MC_SERVER_DIR/server
sudo wget https://launcher.mojang.com/v1/objects/a16d67e5807f57fc4e550299cf20226194497dc2/server.jar -P $MC_SERVER_DIR/server
